...
subnet1_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=${cluster_name}-public" --query "Subnets[0].SubnetId" --output text)
...
subnet2_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=${cluster_name}-private" --query "Subnets[0].SubnetId" --output text)
