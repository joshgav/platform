...
cat ${this_dir}/resources/install-config.template.yaml | envsubst 1> ${workdir}/install-config.yaml
cat ${this_dir}/resources/agent-config.template.yaml | envsubst 1> ${workdir}/agent-config.yaml

## move files from ${this_dir}/extras into cluster manifests
